...
function move_player(y,x){
	if(game_solved==0){
		...
		if((kira_y+y>-1)&&(kira_x+x>-1)&&(kira_y+y<num_rows)&&(kira_x+x<num_columns)){
			switch(game_field[kira_x+x][kira_y+y]){
			case  0:
				game_field[kira_x][kira_y]-=100;
			...
			case 10:
				...
				break;			
			case 20:
				if((game_field[kira_x+2*x][kira_y+2*y]!=1)&&(game_field[kira_x+2*x][kira_y+2*y]!=20)&&(game_field[kira_x+2*x][kira_y+2*y]!= 30)&&(kira_y+2*y >-1)&&(kira_x+2*x >-1)&&(kira_y+2*y <num_rows)&&(kira_x+2*x < num_columns)){
					game_field[kira_x][kira_y]-=100;
					change_image(kira_y,kira_x);
					kira_x += x;
					kira_y += y;		
					game_field[kira_x][kira_y]+=80;
					change_image(kira_y,kira_x);
					game_field[kira_x+x][kira_y+y]+=20;
					change_image(kira_y+y,kira_x+x);
					if(game_field[kira_x+x][kira_y+y]==30){
						box_in_target++;
					}
				}
				break;
...
//    console.log(image_number);
    $('img[name="+image_number+"]').attr({'src':image_object.src});
